...
class Item(object):
    ...
    def __init__(self, name=u'', min=0, max=0, count=0, loop=0):

        self.name = name                # Наименование товара
        self.min = min                  # Минимальный остаток товара
        self.max = max                  # Максимум сколько набирать товара
        self.count = count              # Количество сколько использовать
        self.loop = loop                # Цикл


class BuyItem(object):
    def __init__(self, mode, item, location=u'main', x=1, y=1, count=0):
        self.mode = mode
...
class ValentItem(object):
    def __init__(self, item, location=u'main', x=1, y=1):
        self.location = location
        self.item = item
        self.x = x
        self.y = y
...
class CraftItem(object):
    ...
    def __init__(self, building=u'', craft_id=None, count=None, signal=None):
...
class SkipTask(GameAction):
    def __init__(self, itemId=None,   # @ReservedAssignment
        ...
        self.action = 'skipTask'
        self.itemId = itemId
        self.extraId = extraId
        self.type = 'item'


class GetMissions(GameAction):
    def __init__(self):
...
class PaintballTargetTransform(GameAction):
    def __init__(self, objId=None,  # @ReservedAssignment
        ...
        assert (itemId is None
                or isinstance(itemId, unicode))
        self.action = 'transformGameBuilding'
        self.itemId = itemId
        self.objId = objId
        self.type = 'game'


class BuyResource(GameAction):
    def __init__(self, itemId=None,  # @ReservedAssignment
                 count=None):  # @ReservedAssignment
        assert (itemId is None
                or isinstance(itemId, unicode))
...
class FertilizeFruitTree(GameAction):
    def __init__(self, objId=None):  # @ReservedAssignment
        assert (objId is None
                or isinstance(objId, long))
        self.action = 'fertilize'
        self.itemId = 'RED_TREE_FERTILIZER'
        self.objId = objId
        self.type = 'item'


class FertilizePlant(GameAction):
    def __init__(self, objId=None):  # @ReservedAssignment
        assert (objId is None
                or isinstance(objId, long))
        self.action = 'fertilize'
        self.itemId = 'RED_FERTILIZER'
        self.objId = objId
        self.type = 'item'


class CookSpeedup(GameAction):
    def __init__(self, objId=None):  # @ReservedAssignment
        assert (objId is None
                or isinstance(objId, long))
        self.action = 'speedup'
        self.itemId = 'RED_SPEEDUPER'
        self.objId = objId
        self.type = 'item'


class BlackJack(GameAction):
    def __init__(self, objId=None,  # @ReservedAssignment
                 bet=None,  # @ReservedAssignment
                 action=None):  # @ReservedAssignment
        self.id = 25
        self.slotId = 0L
        self.action = action
        self.objId = objId
        self.type = 'blackjack'
        self.featureId = 'BJ_01'
        self.bet = bet


class Magic(GameAction):
    def __init__(self, objId=None):  # @ReservedAssignment
        assert (objId is None
                or isinstance(objId, long))
        self.action = 'magic'
...
class GetKnockTrick(GameAction):
...
class BuildingUpgrade(GameAction):
    type = 'building'
...
class RemotePirateCheckin(GameAction):
    type = 'item'

    def __init__(self, objId=None):  # @ReservedAssignment
        assert (objId is None
                or isinstance(objId, long))
        self.action = 'remotePirateCheckin'
        self.objId = objId
        self.type = 'item'
        self.id = 10


class CaptureEvent(GameAction):
...
class PirateCaptureEvent(GameAction):
    ...
    def __init__(self, objId=None):  # @ReservedAssignment
        ...
        self.action = 'capture'
        self.objId = objId
        self.type = 'pirateCapture'


class PirateChopEvent(GameAction):

    def __init__(self, objId=None,  # @ReservedAssignment
                 instruments=None):  # @ReservedAssignment
        assert (objId is None
                or isinstance(objId, long))
        assert (instruments is None
                or isinstance(instruments, dict))
        self.action = 'chop'
        self.objId = objId
        self.type = 'chop'
...
class RemoveWishListItem(GameAction):

    def __init__(self, index=None):  # @ReservedAssignment
        assert (index is None
                or isinstance(index, long))
        self.type = 'wishlist'
        self.action = 'remove'
        self.index = index


class AddWishListItem(GameAction):

    def __init__(self, itemId=None,  # @ReservedAssignment
                 index=None):  # @ReservedAssignment
        assert (index is None
                or isinstance(index, long))
        assert (itemId is None
                or isinstance(itemId, unicode))
...
class SendFreeGifts(GameAction):

    def __init__(self, msg=None,  # @ReservedAssignment
                 itemId=None,  # @ReservedAssignment
                 userIds=None):  # @ReservedAssignment
        assert (msg is None
                or isinstance(msg, unicode))
        assert (itemId is None
                or isinstance(itemId, unicode))
        assert (userIds is None
                or isinstance(userIds, list))
        self.type = u'gifts'
        self.action = u'sendFreeGifts'
        self.msg = msg
        self.itemId = itemId
        self.userIds = userIds


class SendGiftItem(GameAction):
    def __init__(self, msg=None,  # @ReservedAssignment
                 item=None,  # @ReservedAssignment
                 count=None,  # @ReservedAssignment
...
class SendGift(GameAction):

    def __init__(self, gift=None):  # @ReservedAssignment
        assert (gift is None
                or isinstance(gift, SendGiftItem))
        self.type = u'gift'
        self.action = u'sendGift'
        self.id = 12
        self.gift = gift


class GameExchangeCollection(GameAction):
    type = 'item'
...
class GameSellGameObject(GameAction):
...
class MoveToStorage(GameAction):
...
class GameBuryEvent(GameAction):
    def __init__(self, user=None,  # @ReservedAssignment
                 slot=None):  # @ReservedAssignment
        assert (user is None
                or isinstance(user, unicode))
        assert (slot is None
                or isinstance(slot, long))
        self.type = 'bury'
        self.action = 'bury'
        self.user = user
        self.slot = slot
...
class GameDigOutEvent(GameAction):
    def __init__(self, user=None):  # @ReservedAssignment
        assert (user is None
                or isinstance(user, unicode))
        self.type = 'bury'
...
class GameGetInfo(GameAction):

    def __init__(self, players=None):
        assert (players is None
                or isinstance(players, list))
        self.action = 'getInfo'
        self.type = 'players'
...
class GameCookRecipe(GameAction):
    type = 'item'
...
class PirateEnemyHit(GameAction):
    def __init__(self, objId=None):  # @ReservedAssignment
        assert (objId is None
                or isinstance(objId, long))
        self.action = 'hit'
        self.objId = objId
        self.type = 'pirateEnemy'
...
class GameRemoteHalloweenPumpkin(GameAction):
    type = ''

    def __init__(self, objId=None, itemId=None):  # @ReservedAssignment
        assert (objId is None
                or isinstance(objId, long))
        assert (itemId is None
                or isinstance(itemId, unicode))
        self.action = 'remoteTrickTreating'
        self.itemId = itemId
        self.objId = objId
        self.type = 'item'


class GameRemoteNewYear(GameAction):
    type = ''

    def __init__(self, user=None,  # @ReservedAssignment
                 treeId=None):  # @ReservedAssignment
        assert (user is None
                or isinstance(user, unicode))
        assert (treeId is None
                or isinstance(treeId, long))
        self.user = user
        self.treeId = treeId


class GameRemoteGiftItem(GameAction):
    type = 'newYearTree'
    ...
    def __init__(self, objId=None):  # @ReservedAssignment
        ...
        self.itemId = 'CAKE_PACK_FREE1'
        self.objId = objId
        self.type = 'item'
        self.id = 10


class TradeExchange(GameAction):
    def __init__(self, user=None,  # @ReservedAssignment
                 objId=None,  # @ReservedAssignment
                 give=None,  # @ReservedAssignment
                 want=None):  # @ReservedAssignment
        assert (user is None
                or isinstance(user, unicode))
        assert (objId is None
                or isinstance(objId, long))
        assert (give is None
                or isinstance(give, list))
        assert (want is None
                or isinstance(want, list))
        self.user = user
...
class RemoteTradeExchange(GameAction):
    def __init__(self, user=None,  # @ReservedAssignment
                 objId=None,  # @ReservedAssignment
                 give=None,  # @ReservedAssignment
                 want=None):  # @ReservedAssignment
...
class ChangeLocation(GameAction):
    type = 'gameState'

    def __init__(self, user=None,  # @ReservedAssignment
                 locationId=None,  # @ReservedAssignment
                 travelId=0L):  # @ReservedAssignment
...
class GetNewYearGift(GameAction):
    type = 'newYearTree'

    def __init__(self, objId=None):  # @ReservedAssignment
        self.action = 'applyNewYearGift'
        self.objId = objId
        self.type = 'newYearTree'
        self.index = 0


class GameRemoteFertilizeItem(GameAction):
    type = 'item'

    def __init__(self):
        self.action = 'remoteFertilizeFruitTree'
        self.type = 'item'


class GameRemoteDigItem(GameAction):
    type = 'item'
...
class MonsterDig(GameAction):
    def __init__(self, objId=None):  # @ReservedAssignment
        assert (objId is None
                or isinstance(objId, long))
        self.action = 'startDig'
        self.objId = objId
        self.type = 'item'


class GameRemotePickItem(GameAction):
    type = 'item'
